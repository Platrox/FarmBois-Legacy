        ...
        for i=1, #Config.miningAreas, 1 do
            ...
            if dist <= 2.0 and not mining then
                ...
                if IsControlJustReleased(0, 38) and dist <= 2.0 then
                    lib.hideTextUI()
                    local output = lib.callback.await('farm_bois_platrox:checkAxe', 100, 'axe')
                    if output then
                        mining = true
                        ...
                        while mining do
                            ...
                            if IsDisabledControlJustReleased(0, 24) then
                                ...
                                local rockData = lib.callback.await('farm_bois_platrox:getBoisData', 100)
                                if lib.skillCheck(rockData.difficulty) then
                                    ...
                                    if breakChance < Config.axe.breakChance then
                                        TriggerServerEvent('farm_bois_platrox:axeBroke')
                                        TriggerEvent('farm_bois_platrox:notify', Strings.axe_broke, Strings.axe_broke_desc, 'error')
                                        ClearPedTasks(cache.ped)
                                        textUI[i] = nil
                                        break
                                    end
                                    ClearPedTasks(PlayerPedId())
                                    TriggerEvent('farm_bois_platrox:notify', Strings.failed_mine, Strings.failed_mine_desc, 'error')
                                end
                    ...
                    elseif not output then
                        TriggerEvent('farm_bois_platrox:notify', Strings.no_pickaxe, Strings.no_pickaxe_desc, 'error')
                    end
                end	
...
RegisterNetEvent('farm_bois_platrox:alertStaff')
AddEventHandler('farm_bois_platrox:alertStaff', function()
    TriggerEvent('farm_bois_platrox:notify', Strings.possible_cheater, Strings.possible_cheater_desc, 'error')
end)
